perf.metrics = function(corr.Cuts, dataFullCols, dataLinRECIST, classRECIST) {
    ...
    tmpStarterData = dataFullCols[, tmpStarterColsOfInterest]
...
for (i in 1:nrow(simTimeChangePair)) {
    if (simTimeChangePair[, 2][[i]] <= (1 + tmpCutPR)) {
        simPatientsPR <- c(simPatientsPR, simTimeChangePair[i,])
    }
}
rm(i)
simPatientsPR <- t(matrix(simPatientsPR, nrow = 2))

simPatientsPD = list()
for (i in 1:nrow(simTimeChangePair)) {
    if (simTimeChangePair[, 2][[i]] >= (1 + (tmpCutPD))) {
        simPatientsPD <- c(simPatientsPD, simTimeChangePair[i,])
    }
}
rm(i)
simPatientsPD <- t(matrix(simPatientsPD, nrow = 2))

tThresh=365
### Calculating NPV, PPV, Sensitivity, Specificity
#PPV: 
ppv=sum(simPatientsPR[, 1] > tThresh) / nrow(simPatientsPR)
#NPV:
npv=sum(simPatientsPD[, 1] < tThresh) / nrow(simPatientsPD)
#Sensitivity
sens=sum(simPatientsPR[, 1] > tThresh) / (sum(simPatientsPR[, 1] > tThresh) + sum(simPatientsPD[, 1] > tThresh))
#Specificity
spec=sum(simPatientsPD[, 1] < tThresh) / (sum(simPatientsPD[, 1] < tThresh) + sum(simPatientsPR[, 1] < tThresh))

### Next splitting patients as (PR+SD) vs PD or PR vs (SD+PD)
simPatientsSD = list()
for (i in 1:nrow(simTimeChangePair)) {
    if (simTimeChangePair[, 2][[i]] >= (1 + tmpCutPR) & simTimeChangePair[, 2][[i]]<=(1+(tmpCutPD))) {
        simPatientsSD <- c(simPatientsSD, simTimeChangePair[i,])
    }
}
rm(i)
simPatientsSD <- t(matrix(simPatientsSD, nrow = 2))

# PPVprogfree: (PR+SD) vs PD
ppvProgFree=(sum(simPatientsPR[, 1] > tThresh)+sum(simPatientsSD[, 1] > tThresh))/(nrow(simPatientsPR)+nrow(simPatientsSD))
# NPVnonresp: PR vs (SD+PD)
npvNonResp=(sum(simPatientsSD[, 1] < tThresh)+sum(simPatientsPD[, 1] < tThresh))/(nrow(simPatientsSD)+nrow(simPatientsPD))
# Sensprogfree: (PR+SD) vs PD
sensProgFree=(sum(simPatientsPR[, 1] > tThresh)+sum(simPatientsSD[, 1] > tThresh))/(sum(simPatientsPR[, 1] > tThresh)+sum(simPatientsSD[,1]>tThresh) + sum(simPatientsPD[, 1] > tThresh))                
# Specnonresp: PR vs (SD+PD)
specNonResp=(sum(simPatientsSD[, 1] < tThresh)+sum(simPatientsPD[, 1] < tThresh))/(sum(simPatientsPR[, 1] < tThresh)+sum(simPatientsSD[,1]<tThresh) + sum(simPatientsPD[, 1] < tThresh))
...
pPatientsSD <- tmpPatClass[tmpPatClass$bestClass == 2, c("SurvTimeDays", "bestClass")]
            ...
            if (nrow(pPatientsPR) > 0) {
                if (nrow(pPatientsSD) > 0) {


                    for (i in c(1:nrow(pPatientsPR))) {
                        for (j in c(1:nrow(pPatientsSD))) {

                           
                                            if (pPatientsPR[[i, 1]] >= pPatientsSD[[j, 1]]) {
            ...
            rm(j)
            if (nrow(pPatientsPR) > 0) {
                if (nrow(pPatientsPD) > 0) {


                    for (i in c(1:nrow(pPatientsPR))) {
                        for (j in c(1:nrow(pPatientsPD))) {


                            if (pPatientsPR[[i, 1]] >= pPatientsPD[[j, 1]]) {
                                comparisons <- comparisons + 1
                                sum <- sum + 1
                            } else { comparisons <- comparisons + 1 }
            ...
            for (i in c(1:nrow(pPatientsSD))) {
            for (j in c(1:nrow(pPatientsPD))) {


                if (pPatientsSD[[i, 1]] >= pPatientsPD[[j, 1]]) {
                    comparisons <- comparisons + 1
                    sum <- sum + 1
                } else { comparisons <- comparisons + 1 }
            ...
            #if (nrow(simPatientsPR) > 0) {
                #if (nrow(simPatientsSD) > 0) {
                    #for (i in c(1:nrow(simPatientsPR))) {
                        #for (j in c(1:nrow(simPatientsSD))) {
                            #comparisons <- comparisons + 1
                            #if (simPatientsPR[[i, 1]] > simPatientsSD[[j, 1]]) {
                                #sum <- sum + 1
                            #} else { }
                            #}
                    #}
            ...
            #if (nrow(simPatientsPR) > 0) {
                #if (nrow(simPatientsPD) > 0) {
                    #for (i in c(1:nrow(simPatientsPR))) {
                        #for (j in c(1:nrow(simPatientsPD))) {
                            #comparisons <- comparisons + 1
                            #if (simPatientsPR[[i, 1]] > simPatientsPD[[j, 1]]) {
                                #sum <- sum + 1
                            #} else {
                        #### weight is here: if PRvsPD is wrong, count another comparison
                                #comparisons <- comparisons + 1
                            #}
                            #}
                    #}
            ...
            #if (nrow(simPatientsSD) > 0) {
                #if (nrow(simPatientsPD) > 0) {
                    #for (i in c(1:nrow(simPatientsSD))) {
                        #for (j in c(1:nrow(simPatientsPD))) {
                            #comparisons <- comparisons + 1
                            #if (simPatientsSD[[i, 1]] > simPatientsPD[[j, 1]]) {
                                #sum <- sum + 1
                            #} else { }
                            #}
                    #}
...
c.weighted=sum/comparisons
            ...
            no.simPatientsPR = nrow(simPatientsPR)
            no.simPatientsSD = nrow(simPatientsSD)
            no.simPatientsPD = nrow(simPatientsPD)
...
#Putting all data into data.frame
		data.frame(row=iRow,col=iCol,
            ...
            no.simPatientsPR = no.simPatientsPR,
            no.simPatientsSD = no.simPatientsSD,
            no.simPatientsPD = no.simPatientsPD
            )
...
corr.bestClass$no.simPatientsPR = matrix(tmpParOutput$no.simPatientsPR, nrow = tmpN, ncol = tmpN, byrow = TRUE)
corr.bestClass$no.simPatientsSD = matrix(tmpParOutput$no.simPatientsSD, nrow = tmpN, ncol = tmpN, byrow = TRUE)
corr.bestClass$no.simPatientsPD = matrix(tmpParOutput$no.simPatientsPD, nrow = tmpN, ncol = tmpN, byrow = TRUE)
...
corr.bestClass$no.Patients.comp=list()
    corr.bestClass$no.Patients.comp$PR = corr.bestClass$no.simPatientsPR[corr.bestClass$max.stats$max.indPR, corr.bestClass$max.stats$max.indPD]
    corr.bestClass$no.Patients.comp$SD = corr.bestClass$no.simPatientsSD[corr.bestClass$max.stats$max.indPR, corr.bestClass$max.stats$max.indPD]
    corr.bestClass$no.Patients.comp$PD = corr.bestClass$no.simPatientsPD[corr.bestClass$max.stats$max.indPR, corr.bestClass$max.stats$max.indPD]
