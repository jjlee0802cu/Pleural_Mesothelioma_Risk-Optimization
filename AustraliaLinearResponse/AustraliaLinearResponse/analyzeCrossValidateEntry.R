...
for (iPat in tmpPatList) {
	...
	tmpOutput=perf.metrics.entry(corr.Cuts,tmp.dataFullCols,tmp.dataLinRECIST,classRECIST)
...
cv.Class = dataFullCols[,c(1,30,81,82,128)]
...
cv.perf.bestCuts.bestClass=rcorr.cens(cv.Class$bestCuts.bestClass,
	Surv(cv.Class$EntryTimeDays,cv.Class$Status),outx=TRUE)
...
cv.log.bestCuts.bestClass=survdiff(Surv(EntryTimeDays,Status)~logBestCuts.bestClass,data=cv.Class)
cv.log.bestCuts.firstClass=survdiff(Surv(EntryTimeDays,Status)~logBestCuts.firstClass,data=cv.Class)

cv.log.firstCuts.bestClass=survdiff(Surv(EntryTimeDays,Status)~logFirstCuts.bestClass,data=cv.Class)
cv.log.firstCuts.firstClass=survdiff(Surv(EntryTimeDays,Status)~logFirstCuts.firstClass,data=cv.Class)

save(list=c(
	ls(pattern='cv.+')),
	file=file.path(tmpDataDir,'CVDataSetPerformanceEntry.rdata'))
